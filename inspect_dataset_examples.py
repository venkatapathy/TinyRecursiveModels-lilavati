...
def inspect_dataset(data_dir: str, num_examples: int = 20):
    ...
    if "addition" in data_dir:
        ...
        for part in parts:
            if part.startswith("d") and part[1:].isdigit():
                digits = int(part[1:])
            if part in ["vanilla", "lilavati1", "lilavati2", "lilavati3"]:
                dataset_mode = part
    ...
    for idx in sample_indices:
        ...
        try:
            ...
            carries = None
            if dataset_mode in {"lilavati1", "lilavati2", "lilavati3"} and digits:
    ...
    for ex in examples_by_result[:5]:
        if dataset_mode in {"lilavati1", "lilavati2", "lilavati3"} and ex['carries']:
